...
#         self.fc=nn.Linear(d_k,d_model)
    ...
    def forward(self,x):   #batch,s,d_model
        ...
        context=context.reshape((batch_size,self.n_head,-1))
...
#         context=self.fc(cross_interation)  #batch,s,d_model
        return self.layernorm.to(device)(cross_interation)
...
def run(args):
    ...
    # 文件名
    file_name = os.path.basename(__file__).split(".")[0]
    file_name = f'{file_name}_{args.operation}'
